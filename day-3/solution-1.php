...
foreach ($lookup as $rowIndex => $row) {
	foreach ($row as $columnIndex => $char) {
		if (!is_numeric($char) && $char !== '.') {
			$total += checkForNumberAtIndex($lookup, $rowIndex - 1, $columnIndex);
			$total += checkForNumberAtIndex($lookup, $rowIndex - 1, $columnIndex - 1);
			$total += checkForNumberAtIndex($lookup, $rowIndex - 1, $columnIndex + 1);
			$total += checkForNumberAtIndex($lookup, $rowIndex, $columnIndex - 1);
			$total += checkForNumberAtIndex($lookup, $rowIndex, $columnIndex + 1);
			$total += checkForNumberAtIndex($lookup, $rowIndex + 1, $columnIndex);
			$total += checkForNumberAtIndex($lookup, $rowIndex + 1, $columnIndex - 1);
			$total += checkForNumberAtIndex($lookup, $rowIndex + 1, $columnIndex + 1);
